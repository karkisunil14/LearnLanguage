...
1. **Publishable key** — Read from `process.env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY` (`.env` file). Never `NEXT_PUBLIC_`, never hardcoded. If no key exists, ask the developer for one. `npx clerk@<version> init --framework expo` can provision one instead (it installs the SDK and writes the env file), but since it installs a dependency and writes files, get the developer's explicit go-ahead before running it — confirm a specific, validated CLI version as part of that approval rather than an unpinned `@latest`. Either way, wait for the key (or approval) before editing files.
