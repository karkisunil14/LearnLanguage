...
3. lesson_completed — fires when the user completes a lesson.
   Properties: { lesson_id: string, time_to_complete_seconds: number }
...
- Track completion with a ref (e.g. `hasCompletedRef`) set to true right before/when lesson_completed fires. On unmount, only emit lesson_abandoned if that ref is still false, so a normal completion never also fires an abandonment event.
